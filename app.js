...
// viewport/--vh fix, low-power renderer, reduced-motion + battery saver, touch tuning.
...
async function boot(){
  ...
  function loop(){
    ...
    if(!SZL_REDUCED) scene.rotation.y = t*0.04;   // honor prefers-reduced-motion
    // decay route tubes
    ...
    controls.update(); if(!document.hidden) renderer.render(scene,camera); requestAnimationFrame(loop);   // battery saver when hidden
  }
  loop();
}
